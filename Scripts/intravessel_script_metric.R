...
horizontal_wall_thickness <- read.csv("./Data/horizontal_wall_thickness.csv",
...
wall_thickness <- import('./Data/wall_thickness.csv')
...
group_distance_density_plot <- ggplot(DGroup_three, aes(x = Distance, fill = ware)) +
  ...
  geom_vline(data = wall_thickness %>% group_by(ware) %>% 
             ...
             aes(xintercept = mean_value, color = ware), linetype = "dashed", linewidth = 0.7) +
  scale_color_manual(values = manual_colors) + 
  labs(
